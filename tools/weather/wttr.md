...
This folder no longer ships the upstream `wttr.in` docs as the model-facing
tool. Instead, the weather tool now exposes a deterministic wrapper around
`wttr.in` so the model can request weather without getting full `curl` access.

## Current contract

- Source service: `https://wttr.in`
- Transport: fixed `curl` invocation inside `tools/weather/run.ts`
- Response format: fixed `format=j2`
- Units: fixed metric via `m`
- Language: fixed English via `lang=en`
- Output: normalized JSON only

## Why this file still exists

`wttr.in` remains the upstream data source, so this file acts as a lightweight
note about that dependency. The actual tool behavior is documented in
